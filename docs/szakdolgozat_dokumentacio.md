...
Az alábbi blokkdiagram a monorepó logikai felépítését mutatja: a böngészőben futó React kliens a játéklogikát (`core`) és a hook réteget (`useGameLoop`, `useAIWebSocket`) használja; a Node.js backend REST API-n keresztül kezeli a felhasználókat és pontszámokat SQLite-ban; a Python AI szolgáltatás WebSocketen fogadja a játékállapot-pillanatképet és a stratégia-regiszterből választott algoritmus szerint ad vissza irányt. A benchmark scriptek ugyanebből a stratégia-kódból indulnak ki, eredményeiket JSON és PNG formában tárolják, amelyeket a statisztika nézet HTTP-n keresztül olvas be.

![Rendszer blokkdiagram – frontend, backend, AI service és benchmark](images/blokkdiagram.png)

*3.1. ábra – Blokkdiagram: a négy fő komponens (frontend, backend, AI service, benchmark) és a REST, WebSocket, illetve fájlrendszer-alapú adatkapcsolatok.*
...
Az osztálydiagram három fő réteget emel ki. A **Core** csomag (`game`, `snake`, `food`, `collision`) tartalmazza a pályafüggetlen játéklogikát; a **Hooks** réteg (`useGameLoop`, `useAIGameLoop`, `useAIWebSocket`) köti össze a React életciklust a játékmotorral és az AI WebSocket kapcsolattal; az **UI** komponensek (`App`, `MainMenu`, `GameCanvas`, `Settings`, `Results`, `Statistics`, `Profile`) a képernyők megjelenítéséért felelnek. A `GameConfig` és `GameStateSnapshot` típusok, valamint a `Strategy` interfész (`nextMove`) biztosítják, hogy a beállítások, az állapot-pillanatkép és a stratégia-választás egységesen kezelhető legyen.

![Frontend osztálydiagram – Core, Hooks és UI rétegek](images/osztalydiagram_frontend.png)

*3.2. ábra – Frontend osztálydiagram: a játékmotor (`core`), a React hookok és a felhasználói felület komponensei közötti függőségek.*
...
A `Statistics.tsx` a benchmark eredményeket jeleníti meg; életciklusa külön van a játékhuroktól. Az oldal betöltésekor a komponens a `fetchBenchmarkSummaries()` függvénnyel egy HTTP kérést küld az AI szolgáltatás `GET /benchmark/summaries` végpontjára. A válaszból kiolvassa a stratégiánkénti összefoglaló sorokat (`summaries`), opcionálisan a szerver által használt eredménymappa elérési útját (`benchmark_dir`), majd alapértelmezetten minden stratégiát láthatónak jelöl a szűrőben. Ha a hívás sikertelen (pl. az AI szolgáltatás nem fut), hibaüzenet jelenik meg; ha a felhasználó közben elnavigál az oldalról, a függőben lévő kérés nem írja felül az új állapotot.
...
Játék közben a HUD egy külön sorban jelzi, honnan jön az MI döntés: ha a WebSocket kapcsolat él, a kiválasztott stratégia felhasználóbarát neve látszik (pl. „backend (A*)”); ha nincs kapcsolat, a „helyi” felirat utal a böngészőben futó tartalék algoritmusra. Az egér fölé vitve a szöveget rövid magyarázó tooltip is megjelenik.
...
Az Express alkalmazás (`ExpressApp`) három útvonal-csomagra bontja a REST API-t: `authRoutes` (regisztráció, bejelentkezés), `profileRoutes` (profil lekérdezés és módosítás) és `scoresRoutes` (pontszámok listázása és mentése). A védett végpontok az `authMiddleware` JWT-ellenőrzését használják. Az adatmodell két entitásra épül: a `User` (email, felhasználónév, jelszó-hash) és a hozzá kapcsolódó `Score` rekordok (pont, lépésszám, kígyóhossz, mód, opcionális `ai_strategy`), amelyeket a beágyazott SQLite adatbázis tárol.

![Backend osztálydiagram – Express útvonalak, middleware és adatmodell](images/osztalydiagram_backend.png)

*3.3. ábra – Backend osztálydiagram: REST végpontok, JWT middleware, valamint a `User`–`Score` kapcsolat az SQLite adatbázissal.*
...
Minden stratégia az absztrakt `Strategy` osztály `next_move(state) -> Direction` szerződését valósítja meg. A **heurisztikus** implementációk (A*, BFS, Hamilton, lookahead, minimax stb.) szabály- és keresésalapú döntést hoznak; a **tanult** stratégiák (DQN, PPO, NEAT) előre betöltött modellekből olvasnak ki, tanításuk a `SnakeEnv` környezetben történik. A `GameState` reprezentálja a pillanatnyi pályaállapotot; a `parse_state` és `simulate_step` segédfüggvények a bejövő JSON-ból állapotot építenek, illetve virtuális lépést szimulálnak.

![AI stratégia osztálydiagram – Strategy interfész és implementációk](images/osztalydiagram_ai_strategiak.png)

*3.4. ábra – AI osztálydiagram: a közös `Strategy` interfész, a heurisztikus és tanult implementációk, valamint a `GameState` és `SnakeEnv` kapcsolata.*
...
Az alábbi stratégiák ugyanarra a bemenetre (`GameState`) és kimeneti interfészre (`next_move` → irány) épülnek, ezért benchmarkban közvetlenül összevethetők. Mindegyik a rácsot gráfként kezeli: a kígyó teste (a farok kivételével) és a pálya széle akadály. Közös segédfüggvényeket használnak: útkeresés (A*, BFS), farok felé haladás, flood-fill alapú „szabad tér” mérés, valamint tiltás az azonnali 180°-os visszafordulásra. A döntési logika tipikusan három szinten működik: először célvezérelt lépés (étel felé), másodszor túlélési tartalék (farok követése), harmadszor a legbiztonságosabb lokális lépés.

##### 3.4.2.1 A* (`astar`)

Az A* (A-csillag) egy **heurisztikus útkereső algoritmus**: a pályát gráfként modellezi, a cél az étel cellája. Minden lépésnél kiszámolja, melyik irány vezet a legrövidebb (vagy legalábbis jó) úthoz. A költség a megtett lépések száma; a becslés (heurisztika) az étel **Manhattan-távolsága** — ez gyorsan „rámutat” a jó irányra anélkül, hogy minden lehetséges utat végignézne. Akadálynak a kígyó teste (a farok kivételével, mert az egy lépés múlva elmozdul) és a pálya széle számít.

**Döntési sorrend minden tickben:**
1. Ha van biztonságos A* út az ételig, annak **első lépését** követi.
2. Ha nincs ilyen út (pl. a kígyó teste közbeesik), megpróbál a **farok felé** haladni (`path_to_tail`), hogy ne zárja be magát.
3. Ha ez sem ad érvényes lépést, a **legbiztonságosabb szomszédos cellát** választja: azt, amelyik után a legtöbb üres cella marad elérhető (flood-fill alapú mérés).

**Mikor jó választás:** kiegyensúlyozott baseline — gyorsan közelít az ételhez, de van biztonsági visszalépése, ha a közvetlen út veszélyes. Benchmarkon tipikusan közepesen–magas pont, alacsonyabb éhségarány, mint az óvatos vagy tanulatlan stratégiáknál.

##### 3.4.2.2 BFS

A **szélességi keresés (BFS)** szintén a legrövidebb utat keresi az ételig, de **heurisztika nélkül**: rétegről rétegre halad, minden irányt egyenlő súllyal vizsgál. Egységköltségű rácsmozgásnál garantáltan optimális lépésszámú utat ad, ha létezik út.

**Döntési sorrend:** megegyezik az A*-éval — először BFS út az ételig, majd farok követés, végül legbiztonságosabb lokális lépés.

**Különbség az A*-hoz képest:** nincs Manhattan-becslés, ezért több csomópontot jár be (lassabb számítás), de a döntés minősége gyakran nagyon hasonló. Jól értelmezhető **kontrollstratégia**: „mi történik, ha nem segítünk az algoritmusnak távolságbecsléssel?”

##### 3.4.2.3 FollowTail

A FollowTail **hibrid megközelítés**: nem minden áron megy az étel felé, hanem figyeli, hogy a lépés után marad-e elég szabad tér. Ha van biztonságos út az ételhez, azt követi (A* első lépése); ha nincs, vagy a lépés túl kockázatos lenne, **farokkövetésre** vált — a kígyó a saját farka nyomán halad, így fenntartja a mozgásteret, és később újra nyithat utat az ételhez.

**Döntési sorrend:**
1. A* első lépés az étel felé, ha érvényes és biztonságos.
2. Farok felé haladás első lépése, lehetőleg 180°-os visszafordulás nélkül.
3. Legbiztonságosabb lokális lépés.

**Jellemző viselkedés:** gyakran erős közepes–magas pontszám, mert jól kombinálja a célvezetést és a túlélést. Hosszú kígyónál a farok-követés néha körbe kényszeríti a testet, ami növelheti a fal- vagy önütközés arányát bizonyos fázisokban.

##### 3.4.2.4 Hamilton család (spirál, zigzag, short cycles)

A Hamilton-család **ciklikus pályakövetésen** alapul: előre kiszámít egy determinisztikus bejárást, amely minden cellát érint, és a kígyó ezen a „körön” halad. Az ötlet: ha strukturáltan járja be a pályát, kisebb az esélye, hogy véletlenül csapdába zárja magát. Ételnél **rövidítést** (levágást) enged: ha egy A* út az étel felé biztonságos első lépést ad, azt választja a ciklus helyett.

###### 3.4.2.4.1 Hamilton spirál (`hamilton`)

A pályát **kívülről befelé spirálisan** járja be: először a felső sor, majd a jobb oszlop, az alsó sor, a bal oszlop, és így tovább a belső cellák felé. A kígyó alapból a ciklus következő cellájára lép; ételnél, ha biztonságos, A* alapú levágást alkalmaz.

**Erősség:** kiszámítható, hosszú túlélés; az önütközés viszonylag ritka, ha a biztonsági feltétel szigorú. **Gyengeség:** a levágás nem mindig optimális — néha hosszabb utat tesz meg, mint egy agresszívabb útkereső.

###### 3.4.2.4.2 Hamilton zigzag (`hamilton_zigzag`)

A spirál helyett **soronként váltott irányú sávokban** (bal→jobb, majd jobb→bal) járja be a pályát. A döntési logika megegyezik a spirál változattal: ciklus követése + opcionális A* levágás ételnél.

**Különbség a spirálhoz képest:** más geometriájú bejárás → más hibaminták és eltérő halálarányok lehetségesek ugyanazon pályán.

###### 3.4.2.4.3 Hamilton short cycles (`hamilton_short_cycles`)

A pályát **2×2-es blokkokra** bontja; minden blokkban egy rövid, négy cellás mini-cikluson halad. Ételnél megpróbál a szomszédos blokk felé váltani A* első lépéssel, ha az biztonságos; különben a blokkon belüli ciklust követi, végül a szokásos safety fallback lép életbe.

**Jellemző:** lokális struktúra fenntartása — a blokkos bejárás miatt más teljesítményprofil, mint az egybefüggő spirálnál; erősen függ a pályamérettől és attól, milyen gyakran engedélyezett az étel felé térés.

##### 3.4.2.5 Greedy (`greedy`)

A Greedy a **legszimplább** heurisztika: minden tickben kizárólag a **legbiztonságosabb lokális lépést** választja — azt az irányt, amely után a legtöbb üres cella marad elérhető (flood-fill). **Nem célvezérelt:** nem keresi aktívan az ételt, csak a „legtöbb lélegzet” elvét követi.

**Erősség:** bizonyos helyzetekben stabil túlélés, kevesebb hirtelen önbeütközés. **Gyengeség:** könnyen köröz vagy nagyon lassan növekszik → benchmarkon gyakran **alacsony pont** és **magas éhség** arány (a futás időkorlát miatt leáll, mielőtt sokat enne).

##### 3.4.2.6 Max Safety (`max_safety`)

A Max Safety a Greedy **szigorúbb változata**: csak olyan lépést enged meg, amely után **továbbra is létezik út a fej és a farok között** — vagyis a kígyó nem vágja el a saját „kijáratát”. A megmaradt jelöltek közül azt választja, amelyik a legtöbb szabad teret hagyja.

**Döntési sorrend:**
1. Minden érvényes, nem 180°-os irány szimulálása.
2. Fej–farok összeköttetés ellenőrzése (van-e út a testen keresztül).
3. A megfelelő lépések közül maximum flood-fill.

**Jellemző viselkedés:** extrém konzervatív — hasonlóan a Greedy-hez, sok futásban alig eszik, ezért alacsony pont és magas éhség gyakori. Ha mégis növekszik, hosszú, óvatos játékot látunk.

##### 3.4.2.7 Lookahead 1 (`lookahead`)

Egy lépésre **előretekint**: minden legális irányra kiszámítja, hány üres cella maradna elérhető a kígyó számára az adott lépés után (flood-fill, a farok mozgását is figyelembe véve). A **legnagyobb teret** adó irány nyer; döntetlen esetén az **ételhez közelebb** eső irány dönt.

**Miért hatékony:** olcsó számítás (csak egy lépés előre), mégis lokálisan jól kerüli a zsákutcát, és célorientált tie-breakkel rendelkezik. Benchmarkon gyakran a **legerősebb heurisztikák** között szerepel: magas átlagpont, relatíve alacsony éhség.

##### 3.4.2.8 Lookahead N (`lookahead_3`, `lookahead_5`)

Több lépésre előre szimulál: minden első irányra végigjátssza a következő N lépést (belső döntéseknél egyszerűsített greedy szabályokat használ), majd az előrejelzett végpontot értékeli. Az értékelés figyelembe veszi a szabad teret és az étel távolságát (súlyozott kombináció).

- **`lookahead_3`:** három lépés horizont — gyorsabb, jó kompromisszum a mélység és a sebesség között.
- **`lookahead_5`:** öt lépés horizont — messzebbre „lát”, de drágább számítás; bizonyos beállításokban **túl konzervatív** lehet (lassabban megy az étel felé).

**Összehasonlítás:** több lépés = jobb zsákutcakerülés, mint 1 lépésnél, de nagyobb számítási költség; a `lookahead_5` nem mindig jobb a `lookahead_3`-nál, mert a mélyebb szimuláció hibás becslései is torzíthatják a döntést.

##### 3.4.2.9 Minimax (`minimax`)

Rövid horizonton (1–2 lépés) **maximin** elv szerint dönt: minden lehetséges első lépésre feltételez egy „legrosszabb” folytatást a második lépésben, majd azt az első lépést választja, amelynek a legrosszabb esete is a legjobb. Az állapotot számmal értékeli (szabad tér, étel távolság, ütközés elkerülése).

**Szándék:** konzervatív, robusztus döntés — nem az átlagos, hanem a **legrosszabb** folytatások ellen is védekezik. **Korlát:** a horizont rövidsége miatt nem lát messzire; zsúfolt végjátékban instabilabb lehet, mint az 1 lépéses lookahead, és a pontszám szórása nagyobb lehet.
...
Az AI szolgáltatás két módon „beszélget” a böngészővel: **REST** hívásokkal (egyszeri kérdés–válasz) és **WebSocket** kapcsolattal (folyamatos, nyitott csatorna). A REST hasonló egy rövid telefonhíváshoz: elküldünk egy kérést, megvárjuk a választ, majd a kapcsolat lezárul. A WebSocket inkább egy nyitott vonal: a játék alatt minden lépésnél új üzenet megy ki és jön vissza anélkül, hogy minden alkalommal újra „fel kellene hívni” a szervert.

**REST végpontok — mikor mire valók**

| Végpont | Mit csinál (közérthetően) |
|---------|---------------------------|
| `GET /health` | Ellenőrzi, hogy az AI szolgáltatás fut-e. |
| `GET /strategies` | Visszaadja a választható MI-stratégiák listáját (pl. A*, DQN). |
| `POST /next` | Egyetlen lépésre kér irányt: a játék aktuális állapotát JSON-ben küldjük, a válaszban jön az irány. Tesztelésre és egyszeri hívásokra jó. |
| `GET /benchmark/summaries` | Összefoglaló statisztikákat ad vissza a stratégiák összehasonlításához ([6.11](#h-6-11)). |
| `GET /benchmark/plots/{filename}` | A benchmarkból készült diagramokat (PNG) szolgálja ki ugyanerre az oldalra. |

A benchmark végpontokhoz a szervernek elérhetőnek kell lennie a `benchmarks/results/` és `benchmarks/results/plots/` mappáknak — ezekből olvassa be az adatokat és a képeket.

**WebSocket — MI mód játék közben**

Amikor a felhasználó **MI módban** játszik, nem minden lépésnél indul új REST hívás, hanem a játék elején megnyílik a `WebSocket /ws` kapcsolat. Ezután minden játék-lépésnél (tick) a böngésző elküldi a pálya pillanatképét és a kiválasztott stratégia nevét; az AI szolgáltatás kiszámítja a következő irányt (`Up`, `Right`, `Down`, `Left`), és visszaküldi. A kliens ezt az irányt alkalmazza, majd újrarajzolja a pályát.

Ha a WebSocket nem elérhető (pl. az AI szolgáltatás nem fut), a frontend **helyi tartalék** stratégiára vált (Greedy vagy A*), és a HUD jelzi, hogy nem a szerver adja az irányt — így a játék nem áll le teljesen, csak gyengébb MI-rel folytatódik.

Az alábbi szekvenciadiagram egy teljes tick folyamatát mutatja MI módban: a kapcsolat felépítésétől a stratégia választásán át az irány visszaküldéséig.

![Szekvenciadiagram – egy tick MI módban](images/szekvenciadiagram_tick_mi_modban.png)

*3.5. ábra – Szekvencia: MI játék indítása, WebSocket kapcsolat, majd tickenként állapot küldése → stratégia → irány vissza; ha a WebSocket nem elérhető, helyi tartalék lép életbe.*
...
A használati esetek diagram két szereplőt különböztet meg: a **vendég játékost**, aki regisztráció nélkül játszhat (kézi vagy MI mód), beállíthatja a pályát, megtekintheti a lokális eredményeket és a benchmark statisztikákat, valamint válthat témát; és a **regisztrált felhasználót**, aki ezek mellett bejelentkezhet, szerverre mentheti a pontszámát és kezelheti a profilját. Az MI mód játék kiterjeszti a beállításokat stratégia-választással; a benchmark megtekintése magában foglalja a stratégiák táblázatos és grafikus összehasonlítását.

![Használati esetek diagram – Snake + MI webalkalmazás](images/usecase_diagram.png)

*6.0. ábra – Használati esetek: vendég és regisztrált felhasználói szerepkörök, a fő funkciók és az include/extend kapcsolatok.*
...
A HUD ekkor egy **„MI:”** sort jelenít meg a többi statisztika (pont, hossz, lépésszám) mellett. Ha a WebSocket kapcsolat él, a sorban a szerver oldali stratégia **felhasználóbarát neve** látszik (pl. „backend (A*)” vagy „backend (DQN)”); ha a kapcsolat megszakadt vagy az AI szolgáltatás nem elérhető, a **„helyi”** felirat jelzi, hogy a böngészőben futó tartalék algoritmus (Greedy vagy A*) adja az irányt. Az egér fölé vitve rövid tooltip is megjelenik: élő kapcsolatnál „ai_service WebSocket”, helyi módnál „Helyi stratégia”. Így a játékos egy pillantással látja, honnan jön az MI döntés — részletes folyamat: [3.4.4](#h-3-4-4).

<a id="h-6-9"></a>
### 6.9 Játék vége és pontszám mentés

A pontszám mentése két szinten történhet: **vendégként** a böngésző helyi tárolójában (`localStorage`), **bejelentkezett felhasználóként** pedig a Node.js backend SQLite adatbázisában is. A két folyamat összekapcsolódik: előbb be kell jelentkezni, hogy a szerver elfogadja a mentést — ehhez a backend JWT tokent ad, amit minden védett kérésnél elküld a kliens.

**Bejelentkezés röviden:** a felhasználó emailt és jelszót ad meg a bejelentkezési űrlapon. A kliens `POST /api/auth/login` kérést küld; a backend ellenőrzi a jelszót (bcrypt), és siker esetén visszaad egy JWT tokent. Ez a token az `AuthContext`-ben tárolódik, és onnantól „bejelentkezve” van a felhasználó.

**Játék vége után:** amikor a játékállapot `GAME_OVER` lesz és a képernyő még `game`:

1. `saveScore` (`io/storage.ts`): pont, tick, hossz, ISO dátum, mód (`player` / `ai`), MI esetén `aiStrategy` — mindig a **helyi** listába kerül.
2. `setScores(loadScores())` — a lokális lista frissítése a felületen.
3. Ha van érvényes JWT, `submitScoreApi` (`POST /api/scores`, `Authorization: Bearer …` fejléccel) — a rekord a szerver `scores` táblájába kerül; hiba esetén csendes `.catch(() => {})`, a helyi mentés ettől még megmarad.

Az alábbi szekvenciadiagram a teljes útvonalat szemlélteti: bejelentkezés → token → játék vége → szerverre küldött pontszám → frissített eredménylista.

![Szekvenciadiagram – bejelentkezés és pontszám mentés](images/szekvenciadiagramm_bejelemtkezes_pontszammentes.png)

*6.9. ábra – Szekvencia: `POST /api/auth/login` és JWT, majd GAME_OVER után `POST /api/scores` a tokennel; a backend SQLite-ba ír, a kliens frissített listát kap.*

**Fejlesztői megjegyzés:** az alkalmazás indulásakor az `App.tsx` meghívja a `clearScores()` függvényt, ami törli a vendég `localStorage` eredménylistáját. Éles vagy demonstrációs használat előtt érdemes eldönteni, megtartjuk-e ezt a viselkedést; különben az oldal újratöltése után a lokális eredmények nem maradnak meg.
...
- **Mért összefoglaló:** `runs`, pálya `rows×cols`, `max_steps`, `seed_base`, átlag/medián pont, átlag lépés, első étel %, halál okok darabszámban.
